...
class ShapeNetParts(torch.utils.data.Dataset):
    ...
    def __init__(self, split):
        ...
        filter_li = list()
        for i in self.items:
            if self.get_point_cloud_with_labels(i):
                filter_li.append(i)
        
        self.items = filter_li
    ...
    def get_point_cloud_with_labels(shapenet_id):
        ...
        filename_pts = str(shape_id)+".pts"
        filename_seg = str(shape_id)+".seg"
        ...
        with open(ShapeNetParts.dataset_path / category_id / "points" / filename_pts, "rb") as fptr:
            points = fptr.readlines()

        with open(ShapeNetParts.dataset_path / category_id / "points_label" / filename_seg, "rb") as fptr:
            part_labels = fptr.readlines()
        
        if len(points) >= 1024:
          indices = np.random.choice([i for i in range(len(points))],1024,False)
        else:
          return None
        
        points_list = list()
        for i in indices:
          temp = list(map(float,points[i].decode().split()))
          points_list.append(temp)
        points_list = np.array(points_list).T
        
        part_labels_list = list()
        for i in indices:
          temp = int(part_labels[i].decode())
          part_labels_list.append(temp)
        part_labels_list = np.array(part_labels_list)

        return points_list, part_labels_list
